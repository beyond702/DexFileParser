...
struct DexCode {
    u2  registersSize;
    u2  insSize;			/* number of input parameters */
    u2  outsSize;			/* number of output parameters */
    u2  triesSize;
...
/**
 * NOTICE: DO NOT release the return pointer!!!
 */
DEX_INLINE const char* getStringById(const DexFile* pDexFile, int index)
{
	/*
	 * String format in Dex:
	 *   len(Leb128 format) | 2020 | string | 00
	 * */
	const u1* ptr = pDexFile->baseAddr + pDexFile->pStringIds[index].stringDataOff;
	while(*(ptr++) > 0x7f);
 ...
 * NOTICE: DO NOT release the return pointer!!!
 */
DEX_INLINE DexTypeId* getTypeId(const DexFile* pDexFile, int index)
{
	return (DexTypeId*)&(pDexFile->pTypeIds[index]);
}

/**
 * NOTICE: DO NOT release the return pointer!!!
 */
DEX_INLINE const char* getStringByTypeId(const DexFile* pDexFile, int idx)
{
	DexTypeId* pTypeId = getTypeId(pDexFile, idx);

	return getStringById(pDexFile, pTypeId->descriptorIdx);
}

/**
 * NOTICE: DO NOT release the return pointer!!!
 */
DEX_INLINE DexFieldId* getFieldId(const DexFile* pDexFile, int index)
{
	return (DexFieldId*)&(pDexFile->pFieldIds[index]);
}

/**
 * NOTICE: DO NOT release the return pointer!!!
 */
DEX_INLINE DexMethodId* getMethodId(const DexFile* pDexFile, int index)
{
	return (DexMethodId*)&(pDexFile->pMethodIds[index]);
}

/**
 * NOTICE: DO NOT release the return pointer!!!
 */
DEX_INLINE DexProtoId* getProtoId(const DexFile* pDexFile, int index)
{
	return (DexProtoId*)&(pDexFile->pProtoIds[index]);
}

/**
 * NOTICE: DO NOT release the return pointer!!!
 */
DEX_INLINE DexTypeList* getProtoParameters(const DexFile* pDexFile, DexProtoId* proto)
{
	if(proto->parametersOff == 0)
		return NULL;

	return (DexTypeList*)(pDexFile->baseAddr + proto->parametersOff);
}

/**
 * NOTICE: DO NOT release the return pointer!!!
 */
DEX_INLINE DexClassDef* getClassDef(const DexFile* pDexFile, int index)
{
	return (DexClassDef*)&(pDexFile->pClassDefs[index]);
}

/**
 * NOTICE: DO NOT release the return pointer!!!
 */
DEX_INLINE DexTypeList* getInterfaces(const DexFile* pDexFile, DexClassDef* class_def)
{
	if(class_def->interfacesOff == 0)
		return NULL;

	return (DexTypeList*)(pDexFile->baseAddr + class_def->interfacesOff);
}

/**********  Class related   ***********/
/**
 * NOTICE: NEED TO release the return pointer!!!
 */
DexClassData* getClassData(const DexFile* pDexFile, DexClassDef* class_def);

DEX_INLINE DexCode* getMethodDexCode(const DexFile* pDexFile, DexMethod* method)
{
	if(method->codeOff == 0)
		return NULL;

	return (DexCode*)(pDexFile->baseAddr + method->codeOff);
}
